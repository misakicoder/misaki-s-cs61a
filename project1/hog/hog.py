...
def swine_align(player_score, opponent_score):
    ...
    if player_score <= 0 or opponent_score <= 0:
        ...
        GCD = decreases_method(player_score,opponent_score)
        if GCD < 10:
            return False
        else:
            return True

def decreases_method(x,y):
    if x > y:
        return decreases_method(x-y,y)
    elif x < y:
        return decreases_method(x,y-x)
    elif x == 1 or y == 1:
        return 1
    else:
        return x
    ...
    def averaged(*args):
        ...
        while i < trials_count:
            if args:
                ...
                sum += roll_dice(1,original_function)
            i += 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    bacon_scores = free_bacon(opponent_score)
    if bacon_scores >= cutoff or extra_turn(score+bacon_scores,opponent_score):
        return 0
    else:
        return num_rolls  # Replace this statement
    # END PROBLEM 11
