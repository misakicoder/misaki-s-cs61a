...
def balanced(m):
    ...
    if length(left(m)) * total_weight(end(left(m)))  == length(right(m)) * total_weight(end(right(m))):
        ...
        if is_mobile(end(left(m))) and is_planet(end(right(m))):
...
def totals_tree(m):
    ...
    "*** YOUR CODE HERE ***"
    if is_mobile(m):
        t = tree(total_weight(m),[totals_tree(end(left(m)))] + [totals_tree(end(right(m)))])
    elif is_planet(m):
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        ...
        part = [replace_leaf(branch,find_value,replace_value) for branch in branches(t)]
        return tree(label(t),part)
