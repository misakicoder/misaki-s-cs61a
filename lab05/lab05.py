...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    standard_city = make_city('standard_city',lat,lon)
    distance_a = distance(standard_city,city_a)
    distance_b = distance(standard_city,city_b)
...
def berry_finder(t):
    ...
    if not is_tree(t):
        ...
        if label(t) == 'berry':
            ...
            return berry_finder(branches(t)) or berry_finder(label(t))
...
def riffle(deck):
    ...
    while i < len(deck) // 2:
        ...
        result.append(deck[(len(deck)//2 + i)])
        i += 1
    return result
...
def add_trees(t1, t2):
    ...
    len_min = min(len(branches_t1),len(branches_t2))
    part1 = [add_trees(branches_t1[i],branches_t2[i]) for i in range(len_min)]
    part2 = branches_t1[len_min:] + branches_t2[len_min:]
    return tree(root,part1+part2)
